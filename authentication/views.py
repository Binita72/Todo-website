...
def register(request):
    if request.method == "POST":
        context = {'hass_error':False, 'data': request.POST}
        ...
        if User.objects.filter(username=username).exists():
            ...
            return render(request, 'authentication/login.html', context, status=409)
